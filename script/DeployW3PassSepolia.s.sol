...
contract DeployW3Pass is Script {
    function run() public returns (address, address) {
        ...
        require(deployerPrivateKey != 0, "PRIVATE_KEY_SEPOLIA not set");
        require(authorizer != address(0), "AUTHORIZER_ADDRESS_SEPOLIA not set");
        require(initialMerkleRoot != bytes32(0), "INITIAL_MERKLE_ROOT not set");
        require(factoryAddress != address(0), "AUTHORIZER_ADDRESS_SEPOLIA not set");
